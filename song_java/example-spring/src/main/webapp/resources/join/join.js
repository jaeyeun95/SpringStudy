...
function phone() {
	...
	if(phone.value === '') {
		...
		if(!phone_chk.test(phone.value)) {
            msg.innerText = '휴재전화 번호를 다시 확인해주세요.(숫자만 입력해주세요.)';
        } else {
			msg.innerText = '';
		}
	}
...
function all_check() {
	...
	if(document.getElementsByName('userid')[0].value === '') {
		...
		msg.innerHTML = '필수 정보를 입력해주세요.';
		return false;
	} else if(document.getElementById('email_msg').value !== '인증번호가 일치합니다.') {
		msg.innerHTML = '이메일 인증번호를 다시 확인해주세요.';
		return false;
	} else if(document.getElementById('phone_msg').value !== '인증번호가 일치합니다.') {
		msg.innerHTML = '휴대전화 인증번호를 다시 확인해주세요.';
		return false;
	} else if(document.getElementById('pw_chk_msg').value !== '') {
